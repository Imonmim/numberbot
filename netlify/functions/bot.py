...
def process_number_purchase(call):
    ...
    try:
        res = requests.post(f"{BASE_URL}/getnum", json={"rid": selected_range}, headers=headers, timeout=10).json()
        if res.get("meta", {}).get("status") == "ok":
            data = res.get("data", {})
            ...
            # Save mapping for auto OTP forward
...
def group_otp_listener(message):
    ...
    # group er message theke number + otp ber koro
    # Example group msg: "8801... : 123456"
...
def check_otp_callback(call):
    ...
    try:
        res = requests.get(f"{BASE_URL}/success-otp?rid={order_rid}", headers=headers, timeout=10).json()
        if res.get("meta", {}).get("status") == "ok":
...
def handler(event, context):
    if event.get('httpMethod') == 'GET':
        return {"statusCode": 200, "body": "Bot running - Auto OTP"}
    try:
        body = event.get('body')
        if body:
